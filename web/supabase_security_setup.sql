...
-- Reviews: moderation flow. Public submits + reads only APPROVED reviews;
-- admin reads all, approves/hides (update) and deletes. See review_moderation.sql.
alter table public.reviews enable row level security;
alter table public.reviews add column if not exists approved boolean;
update public.reviews set approved = true where approved is null;  -- keep existing reviews visible
alter table public.reviews alter column approved set default false; -- new reviews hidden until approved
